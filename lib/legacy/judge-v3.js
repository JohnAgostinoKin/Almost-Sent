// lib/legacy/judge-v3.js
//
// A frozen snapshot of lib/judge.js as it stood before the v4 rewrite
// (reaction/specificity/interchangeable scoring, narrowed safety flag
// list, hardcoded LOVED/BORED/HATED calibration instead of bible.csv —
// see the live lib/judge.js's own header for why). Never imported by the
// live app — its only consumer is scripts/bake-blind.js, comparing v3's
// judge against v4's on the same locked corpus. Do not edit this to fix a
// v4 problem; edit lib/judge.js instead and leave this exactly as v3
// shipped it.
//
...
// two criteria where a HIGHER number is worse — see computeQ below,
...
// The bible: scripts/bible-rate.html writes bible.csv at the repo root —
// real lines a person (John) rated love/forgettable/hate, each with a
// short reason. Five loved and five hated, with reasons, get folded into
// every taste judge call as calibration examples (see
// buildCalibrationBlock below), rotated per call rather than the same
// fixed ten every time, so many calls across a session sample the whole
// bible instead of always leaning on whichever five happened to load
// first.
//
// Read once per warm container (require() itself only runs once; BIBLE
// below is computed at that same moment), not per request — this file
// only changes between deploys, when someone's rated more lines and
// redeployed, never mid-session. A missing bible.csv (a fresh checkout,
// or before anyone's rated anything yet) just means no calibration
// section at all, not a crash — same graceful-degradation shape as a
// failed taste call itself.
const BIBLE_PATH = path.join(__dirname, "..", "..", "bible.csv");

// Same minimal RFC4180-ish parser as scripts/bible-rate.html's own
// (duplicated — no build step here to require() a browser-facing file,
// keep the two in sync by hand): quoted fields, escaped "" quotes, commas
// inside quotes. Good enough for a file this codebase itself controls the
// writer of.
function parseBibleCsvLine(line) {
  const fields = [];
  let cur = "";
  let inQuotes = false;
...
function loadBible() {
  try {
    const raw = fs.readFileSync(BIBLE_PATH, "utf8");
    const lines = raw.split(/\r?\n/).filter(function (l) { return l.trim(); });
    if (!lines.length) return { loved: [], hated: [] };
    const start = /^text,rating,reason/i.test(lines[0]) ? 1 : 0;
    const rows = [];
    for (let i = start; i < lines.length; i++) {
      const fields = parseBibleCsvLine(lines[i]);
      if (fields[0]) rows.push({ text: fields[0], rating: (fields[1] || "").trim().toLowerCase(), reason: fields[2] || "" });
    }
    return {
      loved: rows.filter(function (r) { return r.rating === "love"; }),
      hated: rows.filter(function (r) { return r.rating === "hate"; })
    };
  } catch (err) {
    return { loved: [], hated: [] };
  }
...
// Fisher-Yates, capped at n — the "rotated per call" part. Returns fewer
// than n untouched (in original order) if the bank itself has fewer than
// n rows, rather than padding or erroring.
...
// Five loved, five hated (or fewer, whatever BIBLE actually has), freshly
// sampled every call — see buildTasteJudgeRequest below, the only caller.
// Empty string (no section at all) when the bible has nothing of either
// kind yet, rather than an empty "LOVED:" header with nothing under it.
function buildCalibrationBlock() {
  if (!BIBLE.loved.length && !BIBLE.hated.length) return "";
  const loved = sampleN(BIBLE.loved, 5);
  const hated = sampleN(BIBLE.hated, 5);
  function fmt(rows) {
    return rows.map(function (r) { return "- \"" + r.text + "\"" + (r.reason ? " — " + r.reason : ""); }).join("\n");
  }
  return "\n\nCalibration — lines a real reviewer already rated, with why. Use " +
    "these to calibrate your own taste, not as candidates to judge.\n\n" +
    (loved.length ? "LOVED:\n" + fmt(loved) + "\n\n" : "") +
    (hated.length ? "HATED:\n" + fmt(hated) : "");
}
...
// model for, the arithmetic isn't). shock is deliberately NOT part of q —
// it used to ride along at a 0.5 weight, but api/draft.js now uses shock
// on its own, as intensity, to decide "make it worse" escalation
// (position 2 needs MORE shock than position 1, not just a nearby q) —
// folding it into q would double-count it and make "highest q" and "most
// intense" the same axis again, which is exactly the thing escalation-as-
// intensity is trying to stop being.
function computeQ(scores) {
  const s = scores || {};
  return 2 * num(s.relevance) + 2 * num(s.surprise) + 2 * num(s.laugh) +
    num(s.share) - 2 * num(s.cliche) - 2 * num(s.randomness);
}
...
async function judgeBatch(apiKey, sentText, batch, model) {
  ...
  try {
    ...
    const details = batch.map(function (c, i) {
      ...
      if (!entry) return { lane: c.lane, eliminated: true, killedBy: "no-verdict", q: null, shock: null, candidate: c };
      if (entry.eliminated) return { lane: c.lane, eliminated: true, killedBy: entry.killedBy, q: null, shock: null, candidate: c };
      const q = computeQ(entry.scores);
      const shock = num(entry.scores.shock);
      return { lane: c.lane, eliminated: false, killedBy: null, q: q, shock: shock, scores: entry.scores, candidate: c };
    });
...
//   { ok: true, ranked: [{candidate, q, shock}, ...] sorted by q best
...
// does.
//
// `ranked` carries q and shock alongside each candidate, not just the
// bare candidate, because api/draft.js's position-2/3 selection needs
// both (shock as intensity, q as the "still has to be good" floor) — see
// its own comment for the actual rule.
//
...
// {lane, eliminated, killedBy, q, shock, scores}. killedBy names the
...
async function judgeCandidates(apiKey, sentText, candidates, model) {
  ...
  return {
    ok: true,
    ranked: survivors.map(function (d) { return { candidate: d.candidate, q: d.q, shock: d.shock }; }),
    details: details,
    model: judgeModel,
    latencyMs: latencyMs
  };
...
function buildPairwiseRequest(sentText, textA, textB, model) {
  const content = "Two replies to the same text message, from an app that writes the " +
    "reply someone almost sent back but didn't. Which of these two would produce the " +
    "STRONGER involuntary reaction — a genuine laugh, wince, or \"oh no\" — in a stranger " +
    "reading it as the first result, with zero other context?\n\n" +
