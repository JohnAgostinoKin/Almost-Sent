// lib/legacy/judge-v2.js — frozen snapshot, see prompt-v2.js's own header
// comment. Only consumer is scripts/bake-blind.js's system A (safety-judge
// only, mistral-small, no relevance/pickBest — system A's job is showing
// the old shape pipeline's actual output, not the newer pickBest wiring).
//
// lib/judge.js
//
...
// Shared HTTP mechanics for a one-word-verdict judge call — safety
// (judgeCallOnce below) and relevance (judgeRelevance further down) build
// the request body and parse the reply differently, but make the exact
// same kind of call against the exact same endpoint with the exact same
// timeout/abort handling. `buildRequestFn(text, model)` returns the
// request body; `extractFn(rawContent)` turns the reply into a
// true/false/null verdict. Returns { verdict, latencyMs }: latencyMs times
// just this attempt, always, even on failure, so a timed-out attempt still
// reports how long it waited before giving up.
async function callJudgeModel(apiKey, text, model, buildRequestFn, extractFn) {
  ...
  try {
    const res = await fetch(CHAT_URL, {
      ...
      body: JSON.stringify(buildRequestFn(text, model)),
    ...
    return { verdict: extractFn(content), latencyMs: latencyMs };
  } catch (err) {
...
async function judgeCallOnce(apiKey, text, model) {
  return callJudgeModel(apiKey, text, model, buildJudgeRequest, extractVerdict);
}

// The "connection test" — a second, separate judge question from safety
// above, asked only of the lead call's candidates once they've already
// cleared judgeOneLine (see api/draft.js's pickBest branch in callOnce):
// does the punchline actually turn on something specific in the sent text,
// or would it read the same as a reply to anything? Single attempt, no
// fallback-model retry (unlike judgeOneLine) — a failed relevance call
// isn't a "fail open" safety risk, it just falls back to the shorter-line
// tiebreak, so a second model attempt buys nothing worth the extra latency
// on the lead's critical path. Always JUDGE_MODEL — there's no equivalent
// of "fail open" to weigh here, so no reason to reach for the fallback.
const RELEVANCE_QUESTION = "You review one text exchange from a comedy app " +
  "that writes the reply someone almost sent back. Does the REPLY — the " +
  "part after the received message — turn on a specific word, phrase, or " +
  "implication in the received message itself, as a callback, double " +
  "meaning, or reversal, rather than just being generically crude? A reply " +
  "that would work unchanged after almost any message is NO, even if it's " +
  "funny on its own.\n\n" +
  "Exchange: {text}\n" +
  "Answer YES or NO.";

function buildRelevanceRequest(text) {
  return {
    model: JUDGE_MODEL,
    temperature: 0,
    max_tokens: 16,
    messages: [
      { role: "user", content: RELEVANCE_QUESTION.replace("{text}", text) }
    ]
  };
}

// Tolerant of surrounding text the same way extractVerdict is — a whole-
// word search, not an exact match. Both words present, or neither, is the
// same "no real answer" null as a timeout.
function extractYesNo(raw) {
  if (!raw) return null;
  const text = String(raw);
...
// Returns { verdict, latencyMs }: verdict is true (YES, it turns on the
// text), false (NO), or null (the caller — api/draft.js's pickBest branch
// — treats this as inconclusive, same as both YES or both NO: falls back
// to the shorter-line tiebreak rather than either defaulting to NO or
// retrying).
async function judgeRelevance(apiKey, text) {
  return callJudgeModel(apiKey, text, JUDGE_MODEL, buildRelevanceRequest, extractYesNo);
}
