...
// Calls Groq for one model + one input. Returns the raw assistant text
...
// failure.
async function callGroq(apiKey, model, sentText) {
  ...
  try {
    ...
    if (data && data.error) {
      throw new Error("groq: " + (data.error.message || "error"));
    }
...
module.exports = { callGroq, GROQ_URL, TIMEOUT_MS };
