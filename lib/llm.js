...
// split out from `latencyMs` (which used to silently include it) so a slow
// request's time is attributed to the actual network round trip, not
// wrongly blamed on prompt assembly. `started` defaults to the same instant
// as the prompt timer so a throw from buildRequest itself (before the
// network call even begins) still produces a sane latencyMs in the catch
// below, rather than a stale or undefined one.
async function callLLM(apiKey, model, sentText) {
  ...
  try {
    ...
    return { text: text, latencyMs: latencyMs, finishReason: choice.finish_reason, provider: provider, t_prompt: t_prompt };
  } catch (err) {
    const latencyMs = Date.now() - started;
