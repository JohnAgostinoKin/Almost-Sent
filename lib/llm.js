...
const { buildPrimaryRequest, buildWildcardRequest } = require("./prompt");

const BASE_URL = (process.env.LLM_BASE_URL || "https://openrouter.ai/api/v1").replace(/\/+$/, "");
const CHAT_URL = BASE_URL + "/chat/completions";
// Lowered from 12000, then raised back to 8000 once the primary call
// started writing three premises before its candidates (see
// lib/prompt.js's buildPrimaryPrompt) — real measurement against gpt-5.4
// post-premises: 2.7-5.7s across five different inputs, two of those
// samples already past the old 6000ms ceiling's margin. 8000 keeps the
// same "fail over to FALLBACK_MODEL fast" intent, just re-tuned to the
// call's own new real cost instead of timing out on ordinary variance.
...
// failure.
//
// `genOpts.kind` picks which lib/prompt.js request builder to use:
// "wildcard" for the two-lane raunchy/gross call, anything else (including
// omitted) for the eight-lane primary call — see api/draft.js's handler,
// the only caller that sets this deliberately. scripts/bake.js calls this
// with no genOpts at all, which defaults to the primary builder — it's
// bake-testing the current (v3) primary generator prompt across models,
// not a frozen v2 snapshot (see lib/legacy/ for that).
//
// t_prompt (ms) times the request-builder call (buildPrimaryRequest or
// buildWildcardRequest, see the dispatch above) plus JSON.stringify()
// alone — building the system prompt is pure, synchronous string work, no
// I/O, so this should be near-zero; it's split out from `latencyMs` so a
// slow request's time is attributed to the actual network round trip, not
// wrongly blamed on prompt assembly.
//
...
async function callLLM(apiKey, model, sentText, genOpts) {
  ...
  let ttfbAt = null;
  try {
    const builder = (genOpts && genOpts.kind === "wildcard") ? buildWildcardRequest : buildPrimaryRequest;
    const body = JSON.stringify(builder(model, sentText, genOpts));
