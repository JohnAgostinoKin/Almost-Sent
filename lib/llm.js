...
const { buildWildcardRequest } = require("./prompt");

const BASE_URL = (process.env.LLM_BASE_URL || "https://openrouter.ai/api/v1").replace(/\/+$/, "");
const CHAT_URL = BASE_URL + "/chat/completions";
// Lowered from 12000, then raised back to 8000 once real measurement
// showed 2.7-5.7s across five different inputs, two of those samples
// already past the old 6000ms ceiling's margin (from back when the
// wildcard call still wrote three premises before its candidates — see
// lib/prompt.js's own header on why that's gone now). 8000 keeps the same
// "fail over to FALLBACK_MODEL fast" intent, just re-tuned to the call's
// own real cost instead of timing out on ordinary variance.
...
// Only for a caller that opts in with genOpts.timeoutMs (the nightly
// pre-warm, api/cron/precompute.js): a slow generator like gpt-6-astra
// takes 15-25s a call, which the 8s ceiling above would kill every time.
// The live path never passes timeoutMs, so it never touches this.
const LONG_TIMEOUT_MS = 60000;
const longDispatcher = new Agent({
  connect: { timeout: LONG_TIMEOUT_MS },
...
// failure.
//
// buildWildcardRequest (lib/prompt.js) is the only request builder left —
// GENERATOR_MODEL and its primary-generator builder retired with v6's
// lane cut (see lib/prompt.js's own header). `genOpts` is still whatever
// {lanes, escalate, shown} bag api/draft.js or a script passes through.
//
// t_prompt (ms) times the request-builder call (buildWildcardRequest, see
// the call below) plus JSON.stringify() alone — building the system
// prompt is pure, synchronous string work, no I/O, so this should be
// near-zero; it's split out from `latencyMs` so a slow request's time is
// attributed to the actual network round trip, not wrongly blamed on
// prompt assembly.
//
...
async function callLLM(apiKey, model, sentText, genOpts) {
  // genOpts.timeoutMs (optional, <= LONG_TIMEOUT_MS) overrides the 8s
  // ceiling; genOpts.costSink (optional, {usd}) accumulates OpenRouter's
  // own reported per-call cost so a caller can enforce a spend cap.
  const timeoutMs = Math.min((genOpts && genOpts.timeoutMs) || TIMEOUT_MS, LONG_TIMEOUT_MS);
  ...
  let ttfbAt = null;
  try {
    const body = JSON.stringify(await buildWildcardRequest(model, sentText, genOpts));
    ...
    const res = await fetch(CHAT_URL, {
      ...
      dispatcher: timeoutMs > TIMEOUT_MS ? longDispatcher : dispatcher
    });
    ttfbAt = Date.now();
    ...
    if (genOpts && genOpts.costSink && data && data.usage && isFinite(data.usage.cost)) {
      genOpts.costSink.usd += data.usage.cost;
    }
    ...
    if (isAbort || isDispatcherTimeout) {
      ...
      const timeoutErr = new Error("timeout after " + timeoutMs + "ms (" + reason + ")");
