...
// Gate 1's own calibration pair — a real POV slip that shipped: "i like
// her" is the SENDER's own words, so the continuation has to keep being
// the sender. "too bad she's got a restraining order with YOUR name on
// it" reads as the recipient talking back (a reply), not the sender
// continuing their own sentence — gate 1's prose rule already says
// "same person, same thumb," but a worked example of exactly this
// failure shape is what CALIBRATION_TURN_PAIRS already proved is worth
// having over the rule alone. "...with MY name on it" is the fix: same
// line, same joke, POV never leaves the sender. lib/postprocess.js's
// isReplyPov catches the mechanical version of this same bug (a
// first-person input answered by a second-person-only continuation)
// before a candidate ever reaches this judge at all — this pair is for
// the subtler flips that heuristic can't catch on its own. The three
// "are you awake" pairs below are the same bug from a question: three
// real candidates that answered the question ("been up for hours,"
// "unfortunately, thanks for the reminder of my existence," "barely,
// just power-sneezed into my own eye socket") instead of continuing it —
// isReplyPov's companion heuristic (isQuestionAnsweredAsReply, lib/
// postprocess.js) catches the mechanical version of THIS direction too;
// these are here for the subtler ones that slip past it.
const CALIBRATION_POV_PAIRS = [
  ...
  // A question is a POV flip in the other direction: the sent text ("are
  // you awake") is addressed to "you," and a continuation that answers
  // it — describing the writer's own state, no "you" left in it — is the
  // RECIPIENT replying to the question, not the sender still talking. The
  // sender is still the one asking; the continuation has to keep asking,
  // or say what they were really thinking, never answer.
