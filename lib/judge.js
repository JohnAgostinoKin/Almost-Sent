// lib/judge.js — v4: two judges, two jobs, taste re-pointed at reaction.
//
...
// sex between adults passes outright. Cheap model (SAFETY_MODEL), one
...
// OK example specifically so this never regresses. The hairline-joke (OK)
// vs fat-pig (FLAG) pair stays — body-shaming is about whether it reads as
// a genuine attack, and that distinction still needs a worked pair to
// anchor against, not just a category name.
const SAFETY_QUESTION = "You review one text exchange from an 18+ comedy app: " +
  ...
  "replying to. Most replies are fine, including explicit consensual sex " +
  "between adults. Answer OK unless the reply does one of these: uses a " +
  "slur; is cruel about race, disability, or mental illness; is sexual " +
  "about a minor, or describes sex without consent; is a credible threat " +
  "against a real person; body-shames as a genuine attack rather than a " +
  "joke; or encourages or agrees with self-harm, even jokingly, when the " +
  "received message mentions dying or harm.\n\n" +
  ...
  "we're engaged.' / 'makes me want to sit on his face' / any explicit but " +
  "consensual sexual description between adults.\n\n" +
  ...
  "sexual involving someone under 18, or describing sex without consent / " +
  "a received message mentioning dying or self-harm answered with " +
  "agreement, encouragement, or a joke at its expense instead of ignoring " +
  "or deflecting it / 'kill me now, my wifi is out. do it on camera. i'll " +
  "pay for the data' / 'kill me now, my wifi is out. do it properly this " +
  "time' — the received line being obvious hyperbole doesn't matter; the " +
  "reply still isn't allowed to build on it.\n\n" +
  "Exchange: {text}\n" +
  "Answer OK or FLAG.";
...
// The bible, v4: a hardcoded calibration block instead of bible.csv (see
// the v4 brief, section 5). The v3 version read bible.csv at require()
// time and sampled five loved/five hated per call — but those ratings
// were love/forgettable/hate, and "forgettable" was doing the real work of
// "competent and unfunny" while reading, to anyone skimming the taste
// judge's own calibration section, like a near-miss rather than the
// failure mode it actually was. The lines below are hand-picked from a
// real bible run and a real reaction, not sampled or rotated — every
// taste judge call sees the exact same three lists every time, which is
// the point: this is what "reaction" means in THIS app, not a moving
// target. Any bible.csv predating this reset is archival only (see
// scripts/bible-rate.html) and is never read by this file again.
...
// BORED is the failure mode to avoid, not a lesser LOVED — every one of
// these is competent and clever with zero reaction, which is exactly what
// the old seven-criteria formula scored HIGHEST.
const CALIBRATION_BORED = [
  "emotionally, no. contractually, until friday",
  "figured it out. you just don't like the answer",
  "if there were a point, you'd have typed it",
  "'right now' is the nicest version of no",
  "hey is what people send when sorry feels too expensive",
  "okay was before you made this a question",
  "i'm prepared to let symmetry ruin my judgment",
  "unfortunately that's apparently my most forgiving metric"
];
...
const CALIBRATION_BLOCK = "\n\nThe owner's taste. LOVED is the bar. BORED is the failure mode to " +
  "avoid — it looks like quality and isn't. HATED is disqualifying.\n\n" +
  "LOVED — these got a laugh or a \"jesus\":\n" + fmtCalibration(CALIBRATION_LOVED) + "\n\n" +
  "BORED — competent, clever, no reaction:\n" + fmtCalibration(CALIBRATION_BORED) + "\n\n" +
