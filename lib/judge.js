...
function buildPairwiseRequest(sentText, textA, textB, model) {
  const content = "Two replies to the same text message, from an app that writes the " +
    ...
    "explicit or dark it is? Name the mechanism of each in a few words, then end with exactly " +
  ...
  // first, then the winner, was consistent across both orders 3/3 — the
  // few-words reasoning is what makes the verdict about the lines.
  return {
    model: model,
    temperature: 0,
    max_tokens: 120,
