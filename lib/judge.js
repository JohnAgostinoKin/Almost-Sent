...
// Meant to be the expensive model worth paying for — this is the one call
// per request that decides which three lines a visitor actually sees —
// but Sol measured 9.2s here in production (timing out) even with
// reasoning:{effort:"minimal"} applied (see lib/reasoning.js), so it's
// gpt-5.4 by default instead, same as GENERATOR_MODEL (see lib/prompt.js's
// own comment on that measurement). No fallback model on failure the way
// safety has one: a failed safety call has a real cost (a bad line could
// ship), a failed taste call doesn't — api/draft.js falls back to a fixed
// lane-priority order instead of spending a second call chasing the same
// verdict.
const TASTE_MODEL = process.env.JUDGE_MODEL || "openai/gpt-5.4";
// Lowered from 8000 — a slow model should fail over to the fixed lane-
// order fallback fast, not stall the page waiting on the one call this
// entire request budget can least afford to gamble on.
const TASTE_TIMEOUT_MS = 5000;
