...
function fmtPovPairs(pairs) {
  return pairs.map(function (p) { return "- FLIP (fails gate 1 — the recipient replying, not the sender continuing): \"" + p.flip + "\"\n  PASS (same voice throughout): \"" + p.pass + "\""; }).join("\n");
}
...
function buildTasteJudgeRequest(sentText, candidates, model) {
  ...
  const content = "You review " + candidates.length + " candidate replies to a text message, from an app " +
    ...
    "state, no \"you\" left in it) is the recipient replying too, just from the other direction.\n" + fmtPovPairs(CALIBRATION_POV_PAIRS) + "\n" +
