...
// Similes are banned outright (see lib/prompt.js's "never a simile" rule) —
// this catches the "like a ___" / "as a ___" construction regardless of what
// fills the blank, rather than trying to list every noun a simile could
// reach for the way the string CRUTCHES above do.
const SIMILE_RE = /\b(like|as) an? \w/;
function isCrutch(line) {
  const lower = line.toLowerCase();
  if (SIMILE_RE.test(lower)) return true;
