const { ACTIVE_SHAPES, EXAMPLES } = require("./prompt");
...
// isn't the crime a crutch phrase or a wall word is). One simile in six
// lines reads as a stylistic choice; two reads as the model leaning on the
// crutch. Caught the same "like a ___" / "as a ___" way regardless of what
// fills the blank, rather than trying to list every noun a simile could
// reach for the way the string CRUTCHES list does.
...
function normalizeShape(shape) {
  ...
  return ACTIVE_SHAPES.indexOf(s) !== -1 ? s : "unknown";
}
...
// model thought a line was. Position one (the lead) cycles to the next
// active shape on every call (nextLeadIndex), landing back on the same
// shape every ACTIVE_SHAPES.length-th text, so no one shape is permanently
// favored. The rest of the lines interleave round-robin through the other
// active shapes in the same rotating order, rather than clumping every line
// of one shape before the next — e.g. with shock leading and raunchy
// trailing, the six lines land shock/raunchy/shock/raunchy/shock/raunchy,
// not all three shocks then all three raunchys.
//
...
function nextLeadIndex() {
  leadIndex = (leadIndex + 1) % ACTIVE_SHAPES.length;
  return leadIndex;
}
function orderByShape(items) {
  const leadPos = nextLeadIndex();
  const rotation = ACTIVE_SHAPES.map(function (_, i) { return ACTIVE_SHAPES[(leadPos + i) % ACTIVE_SHAPES.length]; });
  const groups = {};
...
function filterLines(items, sentText) {
  ...
  (items || []).forEach(function (item) {
    ...
    // original. `let`, not `const`: the opener check below can rewrite this
    // in place (prepending ". ") rather than dropping the line.
    let line = normalizePunctuation(stripShapeLabel(String((item && item.text) || "").replace(/\s+$/, "")));
    ...
    if (openerDup || propDup || simileDup) { drop("diversity", function () { droppedDiversity++; }); return; }
