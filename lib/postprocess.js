...
// Narrowed to real cruelty only — appearance/body chatter that isn't
// actually cruel (hair, skin, breath, teeth, face, body, thin, weight) got
// cut; what's left is the stuff that's cruel no matter who reads it: body-
// shaming words themselves (fat/ugly/skinny), slurs, anything involving a
// minor, real-world violence or death (crash/die/kill), and the two prison/
// organ-trafficking cruelty tropes (parole, organs). Whole-word match so
// this doesn't clip unrelated words ("killer playlist" survives; "kill" on
// its own doesn't).
//
...
// outside the \b...\b group (leading boundary only, no trailing one). Kept
// even though the "cut down to real cruelty" pass above didn't name them —
// self-harm content isn't "cruelty" in the same sense the rest of this list
// is, it's a distinct harm this filter has no business loosening.
//
...
const WALL_WORDS = new RegExp(
  "\\b(fat|ugly|skinny|minor|minors|underage|1[0-7][ -]?year[ -]?old|crash|crashes|die|dies|dead|kill|parole|organs?|" +
    SLURS.join("|") +
  ")\\b|\\btherap|\\bsuicid",
  "i"
);
...
function normalizeItem(raw) {
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    return { shape: normalizeShape(raw.shape), text: String(raw.text != null ? raw.text : "") };
  }
  return { shape: "unknown", text: String(raw != null ? raw : "") };
}
...
// order is decided entirely by shape. The lead slot (position one) is
// always the most extreme flavor, shock or raunchy, and which of the two
// leads flips every call (nextLeadShape) so one isn't permanently favored
// over the other. Second is unbelievable, third is absurd, and whichever of
// shock/raunchy didn't lead falls to the back — a normal visitor only ever
// sees the first three (see index.html's MAX_REVEALS), so that's the shape
// the trailing extreme rarely surfaces past ?debug=1.
//
...
// within a group.
let leadToggle = false;
function nextLeadShape() {
  leadToggle = !leadToggle;
  return leadToggle ? "shock" : "raunchy";
}
function orderByShape(items) {
  const lead = nextLeadShape();
  const other = lead === "shock" ? "raunchy" : "shock";
  const rank = [lead, "unbelievable", "absurd", other];
  return (items || [])
...
function filterLines(items, sentText) {
  ...
  (items || []).forEach(function (item) {
    ...
    const line = stripShapeLabel(String((item && item.text) || "").replace(/\s+$/, ""));
    ...
    if (isNonAscii(line)) { drop("ascii", function () { droppedAscii++; }); return; }
