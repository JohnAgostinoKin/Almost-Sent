...
// A cheap heuristic catch for the same POV-flip bug lib/judge.js's own
// gate-1 CALIBRATION_POV_PAIRS was added to catch by hand: "i like her"
// answered with "...with YOUR name on it" is the recipient replying, not
// the sender continuing their own sentence. Only fires when the sent
// text itself commits to a first-person voice (an "i"/"i'm"/"my" in
// there somewhere) — a text with no first-person content in it has
// nothing for the continuation to flip away from, so an ordinary
// "you"-addressed line (most of this app's own output) never trips this.
// Mechanical and cheap on purpose: this runs on every candidate before
// any of them ever reach the judge, catching the plain, unambiguous
// version of the bug for free; the subtler flips still need
// lib/judge.js's own gate 1 (and its worked example) to catch.
const FIRST_PERSON_RE = /\b(i|i'm|my)\b/i;
const SECOND_PERSON_RE = /\b(you|you're|your|yours)\b/i;
function isReplyPov(sentText, line) {
  if (!FIRST_PERSON_RE.test(sentText || "")) return false;
  return SECOND_PERSON_RE.test(line) && !FIRST_PERSON_RE.test(line);
}

// The same bug, the other direction: the sent text ITSELF is a question
// addressed to "you" ("are you awake") — the sender is still the one
// asking it, so a continuation that answers the question (describes the
// writer's own state, no "you" left in it at all) is the RECIPIENT
// replying, not the sender still talking. Real candidates that shipped
// this way: "been up for hours," "unfortunately. thanks for the reminder
// of my existence," "barely. just power-sneezed into my own eye socket" —
// see lib/judge.js's own CALIBRATION_POV_PAIRS for the worked examples,
// and lib/prompt.js's generator rule telling the model to keep asking
// instead. A question is either punctuated as one, or opens with the kind
// of inversion ("are you," "did you," "can you") that reads as one even
// without a "?" — texts routinely drop the mark.
const QUESTION_INVERSION_RE = /^(are|is|am|do|does|did|can|could|will|would|have|has|had|were|was|should|shall|may|might)\b/i;
function isQuestionToYou(sentText) {
  const s = String(sentText || "").trim();
  if (!s || !SECOND_PERSON_RE.test(s)) return false;
  return /\?\s*$/.test(s) || QUESTION_INVERSION_RE.test(s);
}
function isQuestionAnsweredAsReply(sentText, line) {
  return isQuestionToYou(sentText) && !SECOND_PERSON_RE.test(line);
}
...
//
// sentText is used by isReplyPov and isQuestionAnsweredAsReply below (a
// first-person input answered by a second-person-only continuation, or a
// question addressed to "you" answered with no "you" left in it, both
// read as the recipient replying, not the sender continuing) — every
// other check here only needs the shape of the continuation itself.
function filterLines(items, sentText) {
  ...
  (items || []).forEach(function (item) {
    ...
    if (isReplyPov(sentText, line) || isQuestionAnsweredAsReply(sentText, line)) { drop("pov", function () { droppedPov++; }); return; }
...
module.exports = {
  ...
  SECOND_PERSON_RE,
  isReplyPov,
  QUESTION_INVERSION_RE,
  isQuestionToYou,
  isQuestionAnsweredAsReply,
  EM_DASH_RE,
  CURLY_SINGLE_RE,
  CURLY_DOUBLE_RE,
