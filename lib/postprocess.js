...
// Premise-first generation (see lib/prompt.js's buildPrimaryPrompt) changed
// the PRIMARY generator's own output contract from a bare array to
// {premises: [...], candidates: [...]} — this is that shape's own
// extractor, used only for primary calls (see api/draft.js's
// callOneGenerator); the wildcard call still returns a bare array and
// still uses extractArray above unchanged. Same markdown-fence tolerance
// as extractArray, just hunting for the outer {...} instead of [...].
// Returns null on anything that doesn't parse as an object with a real
// `candidates` array — premises is coerced to an array of strings but
// never required (a model that skipped straight to candidates still gets
// its lines judged normally, just with nothing to log for that call).
