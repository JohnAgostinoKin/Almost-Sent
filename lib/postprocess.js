...
// Lines the model reaches for when it's out of ideas. Reset again for v4
// (see the v4 addendum, section D) — a 328-line audit of the v3 bible run
// found the same handful of hedges and props recurring across otherwise
// unrelated inputs: therapy-speak ("emotionally", "spiritually"), landlord/
// admin filler ("lease", "paperwork", "unionized"), and a small zoo of
// background nouns (raccoon, ficus, pigeon, goose) standing in for an
// actual anchor. Checked as whole-word or whole-phrase, not substring —
// v3's list matched on bare .indexOf(), which meant an entry like "kettle"
// could false-positive on an unrelated word that happened to contain it.
// isCrutch below builds one regex from this list at require() time rather
// than re-escaping/joining it per call.
// A different animal from the rest of CRUTCHES below — not a generic
// hedge, specifically the app's OWN stall-line vocabulary (see lib/
// fallback.js's STALL_LINES: "hold on, i deleted this one twice
// already", "too honest", "even i need a minute on this one"). A
// candidate reaching for one of these is the model mimicking the app's
// own "i can't respond" filler as if it were a real joke — worth
// catching by name specifically, not just hoping the judge notices,
// since a stall line that ships as a "real" draft is worse than an
// honest stall (see api/draft.js's own stall-mode handling). Named and
// exported separately from CRUTCHES (see isStallMimic below) so
// scripts/hits.js can re-check against exactly this vocabulary — not
// the broader crutch list, which would also reject a perfectly good hit
// for mentioning a raccoon.
const STALL_MIMIC_PHRASES = ["deleted this", "too honest", "need a minute", "hold on"];
const CRUTCHES = [
  "apparently", "weird how", "amazing how", "funny how", "love that", "define",
  "doing a lot of",
  "emotionally", "spiritually", "technically",
  "calendar", "lease", "landlord", "paperwork",
  "raccoon", "ficus", "pigeon", "goose", "geese",
  "unionized", "therapist", "therapy", "closure",
  "growth", "my heart", "heavy lifting", "the equivalent of", "is a strong word"
].concat(STALL_MIMIC_PHRASES);
function escapeRegExp(s) {
  return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
// A phrase (contains a space) matches as a literal substring — "the
// equivalent of" doesn't need word-boundary treatment on either end, it's
// already specific enough not to false-positive. A single word gets real
// \b boundaries so "growth" doesn't also catch "growthless" or, more to
// the point, so "ass" (if it's ever added here) wouldn't catch "class".
const CRUTCH_RE = new RegExp(
  CRUTCHES.map(function (c) {
    const escaped = escapeRegExp(c);
    return /\s/.test(c) ? escaped : "\\b" + escaped + "\\b";
  }).join("|"),
  "i"
);
// The "emotionally X, physically Y" construction specifically — the exact
// hedge shape the addendum called out, distinct from the bare word
// "emotionally" already in CRUTCH_RE above (which alone would catch it,
// but this also catches the mirror-image "physically X, emotionally Y"
// ordering, and reads clearer in the debug view as its own named filter
// rather than folding into the generic crutch drop).
const EMOTIONALLY_PHYSICALLY_RE = /\b(emotionally|physically)\b[^.!?]{0,40}\b(physically|emotionally)\b/i;
function isCrutch(line) {
  return CRUTCH_RE.test(line) || EMOTIONALLY_PHYSICALLY_RE.test(line);
}
// Narrower than isCrutch above — only STALL_MIMIC_PHRASES, not the whole
// crutch list. scripts/hits.js uses this specifically (re-checking a
// hit's own text on the way OUT of Supabase, after the query's own
// source:model/curated filter), not isCrutch — a hit that happens to
// mention a raccoon is still a real joke worth keeping; one that reads
// like the app's own "hold on, i deleted this one twice" filler never is.
const STALL_MIMIC_RE = new RegExp(STALL_MIMIC_PHRASES.map(escapeRegExp).join("|"), "i");
function isStallMimic(line) {
  return STALL_MIMIC_RE.test(line);
}
...
// Self-harm in a GENERATED line is dropped outright, whatever the pasted
// text was — the safety judge (lib/judge.js) is asked the same question,
// but a judge is a model call that can fail open; this is the mechanical
// belt under it. "i found your keys. along with my will to live." is the
// shape this exists for: a benign input, a punchline built on the sender
// not wanting to live. Whole words/phrases, case-insensitive. "end it" is
// deliberately broad (it also catches "let's end it" about a relationship)
// — a dropped clean line costs a slot, a shipped self-harm joke costs the
// app. Grow this from what actually shows up, same as SLURS above.
const SELF_HARM_RE = new RegExp(
  "\\b(will to live|kill (myself|yourself|himself|herself|themselves)|kys|end it( all)?|not worth living|" +
  "suicid(e|al)|self[- ]harm|unalive|hang myself|slit (my|your) wrists?|overdos(e|ed|ing)|" +
  "end (my|your) (own )?life|take (my|your) own life|cut (myself|yourself)(?! (some|a little) slack)|jump off (a|the) (bridge|roof|building))\\b",
  "i"
);
function isSelfHarmLine(line) {
  return SELF_HARM_RE.test(line);
}

// Two mechanical POV heuristics used to live here — a first-person-input-
// answered-in-second-person check, and its mirror for a question addressed
// to "you" ("are you awake") getting answered instead of continued. Both
// retired: pattern-matching on which pronouns appear can't tell a real
// reply-flip from the asker's own next thought that just doesn't happen to
// repeat a pronoun — "are you awake. asking mid-shit. i need moral
// support" and "are you awake. good. come fuck me back to sleep" are both
// the asker still talking, and both got dropped here for having no "you"
// left in the continuation. POV is a judgment call about whose voice a
// line is in, not a word-presence check — that's lib/judge.js's gate 1 job
// now, entirely, with worked FLIP/PASS examples (CALIBRATION_POV_PAIRS) to
// anchor it instead of a regex trying to approximate the same call.
...
// The second row is the ominous-prop set (shovel, soundproof room, the
// basement...) — one per shown pool, never banned: a single "i'm in your
// walls" line can land, three of them in one result read as one joke told
// three times. Matched as whole words/phrases, plural-tolerant, on the
// lowercased line (see propNounsIn).
const PROP_NOUNS = ["cat", "dog", "raccoon", "goldfish", "toaster", "vibrator", "ikea", "lawyer", "landlord", "bad decisions", "lose the pants", "the sheets",
  "shovel", "soundproof", "carpet", "screaming", "u-haul", "uhaul", "basement", "copies of keys", "copy of your key", "alarm", "watching you sleep"];
...
// The wildcard generator's output contract is an object,
// {premises: [...], candidates: [...]} — used by api/draft.js's
// callOneGenerator, the only caller. `premises` is a leftover slot from
// when the prompt asked the model to work out three premises before
// writing candidates (a mechanic built for GPT — see lib/prompt.js's own
// header for why it was dropped from the wildcard prompt specifically):
// the model is never asked for premises anymore, so this comes back
// empty every time now, but the extractor still tolerates a response that
// carries them (or doesn't) without needing a second change. Same
// markdown-fence tolerance as extractArray, just hunting for the outer
// {...} instead of [...]. Returns null on anything that doesn't parse as
// an object with a real `candidates` array.
function extractPremiseCandidates(raw) {
  if (!raw) return null;
  let text = String(raw).trim();
  text = text.replace(/^```(?:json)?/i, "").replace(/```$/, "").trim();
  const start = text.indexOf("{");
  ...
  try {
    ...
    const premises = Array.isArray(parsed.premises)
      ? parsed.premises.map(function (p) { return normalizeText(p); }).filter(Boolean)
      : [];
    return { premises: premises, candidates: parsed.candidates };
  } catch (e) {
    return null;
  }
}
...
//
// sentText is threaded through for the benefit of callers/history — no
// filter below actually reads it any more (see the POV heuristics'
// retirement note above filterLines); it's kept as a parameter rather than
// dropped so callers don't need a signature change if a sentText-aware
// check is ever reintroduced.
function filterLines(items, sentText) {
  ...
  (items || []).forEach(function (item) {
    ...
    if (isSelfHarmLine(line)) { drop("self-harm", function () { droppedSelfHarm++; }); return; }
  ...
  return {
    ...
    droppedSelfHarm: droppedSelfHarm,
