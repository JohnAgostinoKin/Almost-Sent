const { ACTIVE_SHAPES } = require("./prompt");
...
//
// Entries don't have to be single words — "the human equivalent of" moved
// here from CRUTCHES (it was an outright ban there) because the phrase
// itself isn't the problem, using it twice in the same six-line draw is;
// one line reaching for it is a fine turn of phrase, a second is the same
// bit again. The \bs?\b suffix below is a harmless no-op on a phrase that
// doesn't pluralize (it only ever matters for the single-noun entries).
const PROP_NOUNS = ["cat", "dog", "raccoon", "goldfish", "toaster", "vibrator", "ikea", "lawyer", "landlord", "the human equivalent of"];
...
// (stable — each shape's own queue is only ever shifted from the front).
