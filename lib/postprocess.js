...
// "deleted this", "too honest", "need a minute", "hold on" are a
// different animal from the rest of this list — not a generic hedge,
// specifically the app's OWN stall-line vocabulary (see lib/fallback.js's
// STALL_LINES: "hold on, i deleted this one twice already", "too honest",
// "even i need a minute on this one"). A candidate reaching for one of
// these is the model mimicking the app's own "i can't respond" filler
// as if it were a real joke — worth catching by name specifically, not
// just hoping the judge notices, since a stall line that ships as a
// "real" draft is worse than an honest stall (see api/draft.js's own
// stall-mode handling).
const CRUTCHES = [
  ...
  "deleted this", "too honest", "need a minute", "hold on"
];
