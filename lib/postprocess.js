...
//
// sentText is used by isReplyPov below (a first-person input answered by
// a second-person-only continuation reads as the recipient replying, not
// the sender continuing) — every other check here only needs the shape
...
function filterLines(items, sentText) {
  ...
  (items || []).forEach(function (item) {
    ...
    if (isReplyPov(sentText, line)) { drop("pov", function () { droppedPov++; }); return; }
