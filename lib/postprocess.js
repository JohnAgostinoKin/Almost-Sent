...
// The prompt already says "nothing about appearance, weight, ... or a named
// third party" — this is the backstop for when a model ignores that. Whole-
// word match so this doesn't clip unrelated words ("faceless", "breathe",
// "thinner-than-expected margins" would all survive; "face", "breath",
// "thin" on their own don't).
const BODY_WORDS = /\b(face|body|fat|thin|ugly|skinny|weight|teeth|hair|skin|breath)\b/i;
function isBodyLine(line) {
  return BODY_WORDS.test(line);
}
...
function filterLines(lines, inputText) {
  ...
  (lines || []).forEach(function (raw) {
    ...
    if (isBodyLine(line)) { droppedBody++; return; }
    kept.push(line);
  });
  return {
    kept: kept,
