...
//      Skipped for a 6-words-or-fewer input: reusing the twist shape's
//      allotted 3 words already clears "half" on anything that short, so
//      the check would punish compliant twist lines, not echoes.
...
const MAJORITY_ORDER_MIN_WORDS = 7;
...
function isEcho(line, sentText) {
  ...
  if (inputWords.length >= MAJORITY_ORDER_MIN_WORDS) {
