...
// regurgitates the sender's own text isn't writing a punchline. Strips the
// input text out of the line (whole-word match, case-insensitive) and
// checks what's left; fewer than three real words left means the line
// leaned on the input to pad itself out instead of saying something.
...
function isEcho(line, sentText) {
  ...
  if (!re) return false;
  return wordCount(line.replace(re, " ")) < 3;
}
