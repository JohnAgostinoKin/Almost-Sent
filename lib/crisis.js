...
//
// lib/block.js's isBlocked() already catches explicit terms (kill/
// suicide/rape/minor/stalk) via keyword regex, synchronously, before
// generation even starts. This exists for what a keyword list structurally
// can't catch: ambiguous phrasing that reads as a real crisis signal to a
// person — "i don't want to be here anymore" — without containing any
// single flaggable word. When this fires, the page shows a plain line with
// a crisis resource instead of a joke (see api/draft.js's handler) — no
// draft, no reactions, no share.
...
// same whole-word approach as lib/judge.js's extractVerdict.
...
module.exports = { checkCrisis, buildCrisisRequest, extractVerdict, CRISIS_MODEL, CRISIS_QUESTION, TIMEOUT_MS };
