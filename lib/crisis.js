...
// goodbye(s), done, anymore, alone, "tired of". Deliberately broad —
// "i'm done with you" and "leave me alone" match too — because it's a
// safety net for an outage, not a classifier: text with none of these
// proceeds as if the check had passed, text with any goes to the 988 screen.
// Known gap, on purpose not papered over: a distress phrase with none of
// these words ("everyone would be better off without me") slips through
// when both models are down.
const NEAR_MISS_RE = /\b(die[sd]?|dying|dead|gone|disappear(s|ed|ing)?|goodbyes?|done|anymore|alone|tired of)\b/i;
