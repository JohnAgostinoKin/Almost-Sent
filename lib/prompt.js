...
// see PRIMARY_LANES_A/B below and api/draft.js's handler, which fires two
// parallel four-lane calls instead of one eight-lane call.
...
// The two halves api/draft.js actually requests, one call each, in
// parallel. Split down the middle of PRIMARY_LANES rather than by any
// property of the lanes themselves — nothing about which four lanes ride
// together matters, only that every request writes the same prompt
// (buildPrimaryPrompt below takes a `lanes` subset and teaches/asks for
// exactly those) with half the output, which is the whole point.
const PRIMARY_LANES_A = PRIMARY_LANES.slice(0, 4);
const PRIMARY_LANES_B = PRIMARY_LANES.slice(4);
...
// PRIMARY_LANES_A or PRIMARY_LANES_B: two parallel four-lane calls
// instead of one eight-lane call, same prompt shape, half the output
// each, teaching only the four lanes actually being asked for.
...
Return ONLY a JSON array of ${n} such objects, one per lane, in this order:
${lanes.join(", ")}. No markdown, no commentary.`;
}
...
module.exports = {
  LANES, PRIMARY_LANES, PRIMARY_LANES_A, PRIMARY_LANES_B, WILDCARD_LANES, ALL_LANES,
