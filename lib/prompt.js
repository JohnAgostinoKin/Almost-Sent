...
// The two parallel calls' lane plans for THIS sent text — an invited text
// keeps the raunchy slot in both calls, anything else swaps it for
// deranged. api/draft.js calls this per request; the static WILDCARD_LANE_
// PLAN_A/_B below are the invited shape, for the offline tools that have no
// single sent text to gate on.
function lanePlansFor(sentText) {
  const slot = invitesRaunchy(sentText) ? "raunchy" : "deranged";
  return { invited: slot === "raunchy", A: ["shock", slot, "gross"], B: ["shock", slot, "gross"] };
}
...
const WILDCARD_LANE_PLAN_A = WILDCARD_CALL_A_ORDER.slice();
const WILDCARD_LANE_PLAN_B = WILDCARD_CALL_B_ORDER.slice();
