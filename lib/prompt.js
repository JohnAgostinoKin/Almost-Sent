...
//
// Three real shapes this takes in api/draft.js's two-request generation
// (see the handler there): the full batch above (legacy/bake only now),
// the lead request (shapes: [oneShape], count: 1 — "Write one line. Shape:
// X." replaces the usual "write N covering shapes" instruction entirely,
// since there's no ambiguity left to resolve), and the alternates request
// (shapes: ACTIVE_SHAPES minus the lead, count: 4).
function systemPrompt(sentText, genOpts) {
  ...
  // The lead request (count === 1) gets a flat directive instead of the
  // usual "write N, covering shapes" paragraph — there's exactly one shape
  // and one line, nothing left for the model to distribute or choose.
