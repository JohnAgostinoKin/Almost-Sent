...
function systemPrompt(sentText, wordCap) {
  const cap = wordCap || DEFAULT_WORD_CAP;
  ...
  // comment. Six lines a response, one shape tag each: this is the
  // cheapest real trim available in the output schema itself.
  const shapeEnum = ACTIVE_SHAPES.map(function (s) { return `"${SHAPE_META[s].code}"`; }).join(" | ");
...
function buildRequest(model, sentText) {
  const body = {
    ...
    messages: [
      { role: "system", content: systemPrompt(sentText, wordCapFor(model)) },
