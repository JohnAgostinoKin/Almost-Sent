...
//
// "and at least as funny" is new — the directive used to ask for more
// intense alone ("Not different — worse"), which a purely-more-graphic,
// no-actual-joke line technically satisfied. Same fix as this file's own
// CRUDE_IS_NOT_A_JOKE_RULE and lib/judge.js's matching taste-judge rule:
// intensity without a mechanism isn't the goal here either, escalation
// included.
...
// Hermes only — verbatim, immediately after INTENSITY_LINE. The intensity
// floor above says how far the room can go; this says going there isn't
// the joke by itself — a real answer to what the "crude is not a joke"
// bug actually was: intensity alone (a proposition, a bodily confession,
// a swear word, an insult) with no comic mechanism under it. See lib/
// judge.js's matching rule and calibration pairs for the judge-side half
// of this fix — a prompt rule alone doesn't hold if the model that scores
// the output doesn't also know the difference.
