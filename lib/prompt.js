// lib/prompt.js — v5: two lanes, one room, maximum intensity.
//
...
//
// v5's fix: the first-show room is exactly two lanes, both from Hermes,
// both at maximum intensity — raunchy and gross — nothing else.
// Confession, dark, and absurd still exist (below) but are wired to fire
// ONLY on an escalation refetch, via GENERATOR_MODEL — see api/draft.js's
// handler, which no longer calls GENERATOR_MODEL at all unless escalate
// is true. shock, deranged, and the no-shape "wildcard" lane are retired
// outright, not demoted to escalation-only; they were v4's answer to
// "give Hermes more range," and the audit said the room needed fewer,
// sharper lanes instead. See lib/judge.js's own header for the matching
// safety-judge fix — the flag rate above was a real rubric bug (nano
// over-flagging explicit-but-consensual adult content despite being told
// not to), not a reason to write tamer examples here.
//
// A later latency pass (see api/draft.js's own header) split Hermes into
// two parallel calls and, after that, shrank each call's own count from
// four candidates to three — raunchy went from six to four total, gross
// stayed at two, six candidates total now, not eight (see
// WILDCARD_LANE_COUNTS below) — smaller output per call, faster
// generation, still both lanes represented in both halves.
//
...
const LANES = {
  // --- Hermes' two (WILDCARD_MODEL, split across two calls now — see
  // WILDCARD_LANE_COUNTS below — raunchy x4, gross x2, six candidates
  // total: the entire first-show room, at maximum intensity. See this
  // file's own header for why shock, deranged, and the shapeless
  // "wildcard" lane are gone rather than demoted to escalation-only.
  //
  // Half of each lane's examples below contain "fuck" or "shit" on
  // purpose — the floor for this room is filthy, not merely crude, and an
  // example bank that hedges teaches the model to hedge too.
  raunchy: {
    heading: "RAUNCHY",
    desc: "sexual, filthy, explicit — and it turns on their words",
    group: "wildcard",
    examples: [
      { before: "let me know if you need help", after: ". starting with fucking me before you finish that sentence" },
      { before: "we should catch up soon", after: ". i'll bring the wine, you bring the handcuffs" },
      { before: "you up", after: ". good, no pants, and i'm not asking twice" },
      { before: "call me when you can", after: ", or just send the video. the good one" },
      { before: "sounds good", after: ". so did your sister, twice. shit, so did her roommate" },
      { before: "we should hang out sometime", after: ". or just fuck and stop calling it hanging out" }
    ]
  },
  gross: {
    heading: "GROSS",
...
// Hermes' plan is fixed — the same six-candidate mix every call, first
// show and escalation alike: four raunchy, two gross, in that order.
// Order matters here: it's both what the prompt teaches in and the order
// the model is told to write candidates in. raunchy was 6 (eight total)
// until a latency pass (see this file's own header comment) dropped it to
// 4 so each of the two split calls below writes three candidates instead
// of four — gross stayed at 2 since 1-per-call was already the practical
// floor for that lane.
const WILDCARD_LANE_COUNTS = { raunchy: 4, gross: 2 };
const WILDCARD_LANE_ORDER = ["raunchy", "gross"];
...
// ["raunchy","raunchy","raunchy","raunchy","gross","gross"].
// Still what scripts/bible-prep.js, scripts/audit-prep.js, and scripts/
// bake-blind.js ask for (one Hermes call, all six candidates) — those
// tools aren't latency-sensitive. api/draft.js's handler uses the split
// plans below instead.
const WILDCARD_LANE_PLAN = laneCountsToPlan(WILDCARD_LANE_ORDER, WILDCARD_LANE_COUNTS);

// Split in half for two parallel Hermes calls (see api/draft.js's
// handler) — two raunchy, one gross each (three candidates per call),
// rather than one call writing the whole six-candidate room. Both halves
// are taught from the exact same example bank (formatLaneSection isn't
// split, only the requested count per call is), so this changes how fast
// the room comes back, not what it's taught. Splits the four-and-two
// count evenly since 4/2=2 and 2/2=1 — if either count in
// WILDCARD_LANE_COUNTS ever stops dividing evenly by two,
// laneCountsToPlan's own Math.ceil/floor isn't done here, so revisit this
// split by hand rather than assuming it still halves cleanly.
const WILDCARD_SPLIT_COUNTS = { raunchy: WILDCARD_LANE_COUNTS.raunchy / 2, gross: WILDCARD_LANE_COUNTS.gross / 2 };
const WILDCARD_LANE_PLAN_A = laneCountsToPlan(WILDCARD_LANE_ORDER, WILDCARD_SPLIT_COUNTS);
const WILDCARD_LANE_PLAN_B = laneCountsToPlan(WILDCARD_LANE_ORDER, WILDCARD_SPLIT_COUNTS);
...
// the model should write them — e.g. "6 RAUNCHY, 2 GROSS". Repeats collapse
// to a count so the instruction reads as "write this many of this lane,"
// not the same heading listed six times in a row.
...
// Hermes only — the voice framing for its two lanes.
...
// Hermes generator (raunchy, gross — WILDCARD_MODEL) — this is the entire
// first-show room now, not one seat in a wider pool. `opts.lanes` actually
// matters here now, unlike earlier versions of this function: api/
// draft.js's handler passes WILDCARD_LANE_PLAN_A or _B to split the room
// across two parallel calls (two raunchy, one gross each — three
// candidates per call) instead of one call writing all six — see that
// file's own header comment for why. Omitted (scripts/bake.js and friends
// baking the whole room cold in one call) means the full
// WILDCARD_LANE_PLAN, same as before.
...
function buildWildcardRequest(model, sentText, opts) {
  return Object.assign({
    model: model,
    temperature: 1.0,
    max_tokens: 1100, // three candidates plus three premises — half the room per call now (see api/draft.js's handler); was 2200 for all eight in one call before the split, 1300 for four before this count was lowered
