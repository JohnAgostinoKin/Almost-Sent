...
// Hermes generator (raunchy, gross — WILDCARD_MODEL) — always the fixed
// eight-candidate plan (WILDCARD_LANE_PLAN), first show and escalation
// alike; this is the entire first-show room now, not one seat in a wider
// pool. `opts.lanes` is accepted for signature symmetry with
// buildPrimaryPrompt and scripts/bake.js's own calling convention, but
// this generator's plan never actually varies — see this file's header
// comment for why.
...
function buildWildcardRequest(model, sentText, opts) {
  return Object.assign({
    model: model,
    temperature: 1.0,
    max_tokens: 2200, // eight candidates plus three premises — the bigger of the two calls now
