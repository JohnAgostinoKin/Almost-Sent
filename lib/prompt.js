...
// request (shapes: [oneShape], count: 2 — two candidates of the same shape,
// api/draft.js's own judging then picks the better one), and the alternates
// request (shapes: ACTIVE_SHAPES minus the lead, count: 4).
function systemPrompt(sentText, genOpts) {
  ...
  // two candidates api/draft.js picks the better of) needs its own
  ...
  const writeInstruction = count === 1
    ...
    : shapes.length === 1
      ? `Write ${countWord(count)}, all shape: ${SHAPE_META[shapes[0]].heading}. Two different takes — not two versions of the same joke.`
