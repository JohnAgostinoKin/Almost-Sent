...
// both at maximum intensity — raunchy (six candidates) and gross (two) —
// nothing else. Confession, dark, and absurd still exist (below) but are
// wired to fire ONLY on an escalation refetch, via GENERATOR_MODEL — see
// api/draft.js's handler, which no longer calls GENERATOR_MODEL at all
// unless escalate is true. shock, deranged, and the no-shape "wildcard"
// lane are retired outright, not demoted to escalation-only; they were
// v4's answer to "give Hermes more range," and the audit said the room
// needed fewer, sharper lanes instead. See lib/judge.js's own header for
// the matching safety-judge fix — the flag rate above was a real rubric
// bug (nano over-flagging explicit-but-consensual adult content despite
// being told not to), not a reason to write tamer examples here.
...
const LANES = {
  // --- Hermes' two (WILDCARD_MODEL, one call, eight candidates: raunchy
  // x6, gross x2 — the entire first-show room, at maximum intensity. See
  // this file's own header for why shock, deranged, and the shapeless
...
// Hermes' plan is fixed — the same eight-candidate mix every call, first
// show and escalation alike: six raunchy, two gross, in that order. Order
// matters here: it's both what the prompt teaches in and the order the
// model is told to write candidates in.
const WILDCARD_LANE_COUNTS = { raunchy: 6, gross: 2 };
...
// ["raunchy","raunchy","raunchy","raunchy","raunchy","raunchy","gross","gross"].
// Still what scripts/bible-prep.js, scripts/audit-prep.js, and scripts/
// bake-blind.js ask for (one Hermes call, all eight candidates) — those
...
// handler) — three raunchy, one gross each, rather than one call writing
// the whole eight-candidate room. Both halves are taught from the exact
// same example bank (formatLaneSection isn't split, only the requested
// count per call is), so this changes how fast the room comes back, not
// what it's taught. Splits the six-and-two count evenly since 6/2=3 and
// 2/2=1 — if either count in WILDCARD_LANE_COUNTS ever stops dividing
// evenly by two, laneCountsToPlan's own Math.ceil/floor isn't done here,
// so revisit this split by hand rather than assuming it still halves
// cleanly.
...
// across two parallel calls (three raunchy, one gross each) instead of
// one call writing all eight — see that file's own header comment for
// why. Omitted (scripts/bake.js and friends baking the whole room cold in
// one call) means the full WILDCARD_LANE_PLAN, same as before.
...
function buildWildcardRequest(model, sentText, opts) {
  return Object.assign({
    model: model,
    temperature: 1.0,
    max_tokens: 1300, // four candidates plus three premises — half the room per call now (see api/draft.js's handler); was 2200 for all eight in one call
