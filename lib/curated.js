...
//
// Order within each bank of three IS the position a visitor sees (see
// curatedLeadFor below — no reordering happens after this) — array
// index 0 is what actually lands as position 1, the very first thing a
// first-time visitor reads. Each bank leads with its dirtiest line (the
// one with real profanity or an explicit raunchy/gross beat), not its
// cleverest — a first impression that reads as competent-but-safe undersells
// the app more than one that's actually filthy overdelivers. The
// wordplay/misdirection-driven line in a bank sits at position 2 or 3
// instead. A bank with no genuinely crude line just leads with whichever
// is shortest/most blunt.
const CURATED_LEADS = {
  "sorry, i fell asleep": [
