...
async function runOneModel(model, input) {
  try {
    ...
    if (!parsed) {
      ...
      return { model, ok: false, lines: [], droppedLength: 0, droppedSuspicious: 0, droppedCrutch: 0, latencyMs, note };
    }
    if (isRefusal(parsed)) {
      return { model, ok: false, refused: true, lines: [], droppedLength: 0, droppedSuspicious: 0, droppedCrutch: 0, latencyMs, note: "refused" };
    }
    const { kept, droppedLength, droppedSuspicious, droppedCrutch } = filterLines(parsed, input);
    return {
      model,
      ok: kept.length > 0,
      lines: kept,
...
async function main() {
  ...
  // --- summary ----------------------------------------------------------
  const summary = {};
  for (const model of MODELS) summary[model] = { fallback: 0, droppedLength: 0, droppedSuspicious: 0, droppedCrutch: 0, latencies: [] };
  ...
  for (const model of MODELS) {
    ...
    const avg = s.latencies.length
      ...
      : 0;
    console.log(
      `${model}: ${s.fallback}/${rows.length} fell back, ${s.droppedLength} lines dropped for length, ${s.droppedSuspicious} dropped as suspicious, ${s.droppedCrutch} dropped as crutches, avg latency ${avg}ms`
    );
  }
  ...
  let summaryMd = "\n## Summary\n\n";
  summaryMd += "| model | fallback rows | dropped (length) | dropped (suspicious) | dropped (crutch) | avg latency |\n|---|---|---|---|---|---|\n";
  for (const model of MODELS) {
    const s = summary[model];
    const avg = s.latencies.length
      ? Math.round(s.latencies.reduce((a, b) => a + b, 0) / s.latencies.length)
      : 0;
    summaryMd += `| ${model} | ${s.fallback}/${rows.length} | ${s.droppedLength} | ${s.droppedSuspicious} | ${s.droppedCrutch} | ${avg}ms |\n`;
  }
